...
```text
preflight → 변경 축 분류 → 작업 DAG/파일 소유권 배정 → 병렬 구현
→ 독립 verifier 판정 → 통합 → ./scripts/verify.sh
→ 목적별 commit/push → CI PostgreSQL 검증 → SHA 이미지 발행
→ Render 자동 배포 → deploy ID/SHA 대조 → 운영 smoke
```

Preflight에서 `git status`, 현재 branch와 HEAD, 기존 사용자 변경, 예약된 공유 파일, 최신 Flyway 번호를 확인합니다.
각 워커에는 아래 작업 패킷을 전달합니다.
...
구현 계약(API·DB·오류·보안)
필수 검증 명령
완료 보고: 변경 파일, 테스트, 미검증 범위, 위험
```

워커 상태는 `in_progress`, `completed`, `blocked`로 관리합니다. 차단 시 리더는 범위를 한 번 좁혀
재지시하고, 해결되지 않으면 작업을 회수하거나 대체 역할에 재배정합니다.
...
# 전체 빌드, 테스트, diff hygiene
./scripts/verify.sh

# 배포 후 Health, OpenAPI, CORS, 인증 경계
...
CI는 pull request에서 전체 Gradle 빌드와 PostgreSQL 16 기반 Flyway/API smoke를 수행합니다. master에서는
검증을 통과한 정확한 commit SHA 이미지만 발행하고 Render deploy ID가 `live`가 된 뒤 운영 smoke를 실행합니다.
기존 인스턴스의 Health만으로 새 배포 성공을 판정하지 않습니다.

## 운영 실패 복구

- 이미지 배포 또는 Health/API 검증 실패: 신규 배포를 실패 처리하고 직전 live 이미지를 유지하거나 재배포합니다.
- Flyway 실패: 트랜잭션 롤백과 schema history를 확인하고, 적용된 migration은 수정하지 않으며 새 migration으로 forward-fix합니다.
- 데이터 backfill: 배포 전후 row count와 점수·상태 분포를 확인합니다.
- DB 외부 접근이 필요하면 현재 IP만 임시 허용하고 검증 직후 allowlist를 원복합니다.
- 비밀값은 명령 출력, 워커 메시지, CI 로그, 완료 보고에 포함하지 않습니다.
